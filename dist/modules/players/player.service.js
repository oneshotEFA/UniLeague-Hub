...
class PlayerService {
    ...
    // creating a player
    async createPlayer(name, position, number, teamId, playerPhoto) {
        try {
            ...
            let avatar = null;
            if (playerPhoto) {
                avatar = await this.galleryService.savePicture(playerPhoto.buffer, player.id, "PLAYER", "AVATAR", true);
            }
            return {
                ok: true,
                data: {
    ...
    async getPlayers(teamId) {
        try {
            ...
            for (const player of players) {
                const avatar = await this.galleryService.getImagesByOwner("PLAYER", player.id, "AVATAR");
                result.push({
                    ...player,
    ...
    async getPlayerById(id) {
        try {
            ...
            const avatar = await this.galleryService.getImagesByOwner("PLAYER", id, "AVATAR");
            return {
                ok: true,
                data: {
                    ...player,
    ...
    // update player
    async updatePlayer(id, number, name, position, avatar) {
        try {
            if (!id) {
                return {
                    ok: false,
            ...
            if (avatar) {
                ...
                await this.galleryService.savePicture(avatar.buffer, id, "PLAYER", "AVATAR", true);
    ...
    // player transfer
    async playerTransfer(playerId, newTeamId, newNumber, tournamentId, managerId) {
        try {
            ...
            event_bus_1.eventBus.emit(events_1.TRANSFER_NEWS, {
    ...
    async playerStatHandler({ eventId }) {
        console.log("bye");
        await db_config_1.prisma.$transaction(async (tx) => {
            ...
            switch (event.eventType) {
                case "Goal":
                    ...
                    break;
                case "Yellow":
                    await (0, utility_1.handleYellowCard)(tx, event);
                    break;
                case "Red":
                    await (0, utility_1.handleRedCard)(tx, event);
                    break;
            }
