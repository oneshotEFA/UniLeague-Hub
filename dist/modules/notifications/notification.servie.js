...
class NotificationService {
    ...
    // send notification
    async sendNotification(senderAdminId, message, reciveradminId) {
        try {
            if (!senderAdminId) {
                return {
                    ok: false,
            ...
            const notification = await this.prismaService.notification.create({
                data: {
                    type: prisma_1.NotificationType.DIRECT_MESSAGE,
                    message: message,
                    senderAdminId,
    ...
    // broadcasting
    async broadCastToEachAdmin(senderAdminId, content) {
        try {
            if (!senderAdminId) {
                return {
                    ok: false,
            ...
            const createNotifications = await Promise.all(receivers.map((admin) => this.prismaService.notification.create({
                data: {
                    type: prisma_1.NotificationType.DIRECT_MESSAGE,
                    meta: content,
    ...
    async broadCastToTournament(senderAdminId, tournamentId, content, photo) {
        try {
            if (!senderAdminId) {
                return {
                    ok: false,
            ...
            const notfication = await this.prismaService.notification.create({
                data: {
                    type: prisma_1.NotificationType.TOURNAMENT_UPDATE,
                    senderAdminId,
            ...
            if (photo) {
                await this.galleryService.savePicture(photo.buffer, notfication.id, "TOURNAMENT", "BANNER");
            }
            return {
                ok: true,
                data: notfication,
    ...
    async getAdminNotification(adminId) {
        try {
            if (!adminId) {
                return {
                    ok: false,
    ...
    async getBroadCastNotification(page = 1) {
        try {
            const limit = 10;
            const skip = (page - 1) * limit;
            const where = {
                type: prisma_1.NotificationType.BROADCAST,
            };
            ...
            const result = await Promise.all(notifications.map(async (not) => {
    ...
    async getTournamentBroadCast(tournamentId, page = 1) {
        try {
            const limit = 10;
            const skip = (page - 1) * limit;
            const where = {
            ...
            const [notifications, totalItems] = await Promise.all([
                this.prismaService.notification.findMany({
                    where: { type: prisma_1.NotificationType.TOURNAMENT_UPDATE, tournamentId },
                    orderBy: {
                        createdAt: "desc",
                    },
                    select: {
    ...
    // delete News
    async deleteBroadCast(notifcationId) {
        try {
            if (!notifcationId) {
                return {
                    ok: false,
    ...
    // update news
    async updateBroadCast(notificationId, content) {
        try {
            if (!notificationId) {
            ...
            const oldMeta = typeof notification.meta === "object" && notification.meta !== null
                ? notification.meta
                : {};
            const updatedMeta = {
                ...oldMeta,
            ...
            const updatedNotification = await this.prismaService.notification.update({
                where: { id: notificationId },
                data: {
                    type: content.type ?? notification.type,
                    meta: updatedMeta,
                },
            });
    ...
    async broadCastToWeb(content, image) {
        try {
            if (!content) {
                return {
                    ok: false,
            ...
            if (image?.buffer) {
                const post = await this.galleryService.savePicture(image.buffer, broadCast.id, "WEB", "COVER", true);
            }
            return {
                ok: true,
                data: broadCast,
    ...
    async systemCall(content) {
        try {
            ...
            const superAdmins = await this.prismaService.admin.findMany({
                where: { role: prisma_1.AdminRole.superAdmin },
                select: { id: true },
            });
    ...
    //send email
    async sendMaintenanceEmail(notifcationId) {
        try {
  ...
  `;
    ...
    async sendEmailToCoach(credentials) {
        try {
            const { recipientName, teamName, tournamentName, registrationKey, accessKey, email, } = credentials;
            const info = await mail_config_1.default.sendMail({
                from: `"UniLeague Hub" <${process.env.ADMIN}>`,
                to: email,
                subject: `You’ve been assigned as Manager for ${tournamentName}`,
                html: (0, utility_1.generateTeamAccessEmailHTML)({
                    recipientName,
                    teamName,
                    tournamentName,
                    registrationKey,
