...
class TeamService {
    ...
    async createTeam(teamName, coachName, coachEmail, logo) {
        try {
            if (!teamName || !logo) {
                return {
                    ok: false,
            ...
            const logoSaved = await this.galleryService.savePicture(logo.buffer, team.id, "TEAM", "LOGO", true);
            return {
                ok: true,
                data: {
                    team,
    ...
    async updateTeam(id, data, logo) {
        try {
            ...
            if (logo) {
                ...
                await this.galleryService.savePicture(logo.buffer, id, "TEAM", "LOGO", true);
    ...
    // search team
    async searchTeamByName(teamName) {
    ...
    async teamStatus(id) {
        try {
            ...
            return {
                ok: false,
                error: (0, utility_1.getUserFriendlyError)(error),
            };
        }
    }
    async getAllTeams() {
