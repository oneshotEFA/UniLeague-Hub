...
export class AdminControl {
  ...
  static async assignManagerToTournament(req: Request, res: Response){
    ...
    const manager = await adminService.assignManagerToTournament(id,tournament)
    if (!manager.ok){
         return res
        .status(400)
        .json(
            new ApiResponseBuilder()
            .badRequest(manager.error)
            .build(res)
        )
    }
    return res
  ...
  static async removeTournamentManager(req: Request, res: Response){
    ...
    if (!removeManager.ok){
        ...
        .json(
            new ApiResponseBuilder()
            .badRequest(removeManager.error)
            .build(res)
        )
    }
    return res
